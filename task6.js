...
const getMatchedNumbers = (firstArr, secondArr) => {
    ...
    for (let i in secondArr) {
        if (firstArr.includes(secondArr[i]) && !res.matched.includes(secondArr[i])) {
            res.matched.push(secondArr[i])
        }
    }

    const unitedArray = firstArr.concat(secondArr);

    for (let i in unitedArray) {
        if (!res.matched.includes(unitedArray[i]) && !res.different.includes(unitedArray[i])) {
            res.different.push(unitedArray[i])
        }
    }
